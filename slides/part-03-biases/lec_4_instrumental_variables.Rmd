...
- When effects are plausibly constant, we cleanly estimate the effect; otherwise, a little more complicated.
...
- Get $\hat{T_i} = f(Z_i, X_i)$ where $\hat{T_i} \bot u_i$. So $\hat{T_i}$ is only influenced by $Z_i$ and $X_i$ (which we already control for). 
...
![list_of_iv](./assets/angrist_list_of_iv.png)
...
- sometimes, the effect on compliers is the causal effect of interest
  - this is local only to those who comply with the instrument though!
...
*discontinuity that leads to a jump in probability of treatment, with $\epsilon$ change*
