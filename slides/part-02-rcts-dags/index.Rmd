...
- The edge represents statistical association, which can exist due to one or more of the following reasons:
    ...
    1. **Selection**: $X$ and $Y$ have a common effect that we are conditioning on
...
- Reverse causation (3) can really only be addressed through temporal ordering.

--

- So we are going to focus on **confounding** and **selection**.

---
#  The Structure of Bias: Confounding
...
---
# The Structure of Bias: Selection/Collider Bias

.footnote[.red[*]Ch 16 by Maria Glymour in Methods Social Epidemiology]

- Collider bias is "the change in association ebtween two variables when conditioning on their common effect". 
...
- Recall the five ways we can find a statistical association:
    1. ~~Just random luck~~
...
- Luck can be addressed by using a large enough sample size to evaluate confidence intervals of the effects.

- Reverse causation can really only be addressed through temporal ordering.
...
- RCTs address most of these
    ...
    - Temporal ordering excludes (3)
    - Randomization excludes (4)
