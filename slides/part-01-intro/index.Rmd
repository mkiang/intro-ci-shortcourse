...
--
.right-column[
If I take Drug B and get a headache, did Drug B *cause* my headache?
]
...
--
.right-column[
Implicitly, Drug B only caused the headache if I would *not* have received the headache if I had not taken Drug B.
]
--
.right-column[
So a causal effect is a comparison of two *potential outcomes* (or *counterfactuals*) on a unit (or a single set of units).
]
---
layout: false
.left-column[
...
.right-column[
So, we define $Y_0$ as my outcome (headache) if I had *not* taken the drug and $Y_1$ as my outcome if I *had* taken the drug
]
???
So let's formalize this with a little bit of notation.
--
.right-column[
Then $Y_0$ and $Y_1$ are my *potential outcomes* or *counterfactuals*.
]
--
.right-column[
And $\tau=Y_1 - Y_0$ is the *individual causal effect* of Drug B.
]
--
.right-column[
...
--
.right-column[
If we knew every outcome, our data would look like this:

| Name | Treatment ($X$) | Outcome ($Y$) | $Y_0$ | $Y_1$ |
|------|:--------------:|:-------------:|:-----:|:-----:|
| Matt  | 1             | 1           | 1   | 1   |
| Marla | 1             | 0           | 1   | 0   |
| ...   | ...           | ...         | ... | ... |
...
--
.right-column[
Here, treatment $X$ is what *actually* happened and outcome $Y$ is the outcome under what actually happened. 
]
--
.right-column[
If we magically had these data, we could just take $\bar\tau=\frac{1}{n}\sum(Y_{i1}-Y_{i0})$ to get the average causal effect
]
---
layout: false
.left-column[
...
--
.right-column[
So causal inference is often described as a **missing data** problem.
]
---
layout: false
.left-column[
...
<img src="./assets/basic_dag.jpg" alt="X to Y" style="width: 30%;"/>
</div>
]
---
layout: false
...
## Likewise, as we enter an era of "big data", causal inference needs data science

## Big data can be very helpful — but it can also make the problem worse with inappropriate methods, violated assumptions, and poor design
